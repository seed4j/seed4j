package com.seed4j.module.domain.properties;

import org.apache.commons.lang3.StringUtils;

public record Seed4JBasePackage(String basePackage) {
  private static final String DEFAULT_PACKAGE = "com.mycompany.myapp";
  public static final Seed4JBasePackage DEFAULT = new Seed4JBasePackage(DEFAULT_PACKAGE);

  public Seed4JBasePackage(String basePackage) {
    this.basePackage = buildBasePackage(basePackage);
  }

  private String buildBasePackage(String basePackage) {
    if (StringUtils.isBlank(basePackage)) {
      return DEFAULT_PACKAGE;
    }

    return basePackage.replace('/', '.');
  }

  public String get() {
    return basePackage();
  }

  public String path() {
    return basePackage().replace('.', '/');
  }

  @Override
  public String toString() {
    return basePackage;
  }
}
