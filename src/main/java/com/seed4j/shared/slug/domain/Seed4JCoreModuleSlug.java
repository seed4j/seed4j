...
import static com.seed4j.module.domain.resource.Seed4JModuleRank.RANK_A;
import static com.seed4j.module.domain.resource.Seed4JModuleRank.RANK_B;
import static com.seed4j.module.domain.resource.Seed4JModuleRank.RANK_C;
import static com.seed4j.module.domain.resource.Seed4JModuleRank.RANK_D;
import static com.seed4j.module.domain.resource.Seed4JModuleRank.RANK_S;
...
public enum Seed4JCoreModuleSlug implements Seed4JModuleSlugFactory {
  ...
  SEED4J_EXTENSION("seed4j-extension", RANK_A),
  SONARQUBE_JAVA_BACKEND("sonarqube-java-backend", RANK_A),
  SONARQUBE_JAVA_BACKEND_AND_FRONTEND("sonarqube-java-backend-and-frontend", RANK_A),
  SONARQUBE_TYPESCRIPT("sonarqube-typescript", RANK_B),
  ...
  SPRING_BOOT_DOCKER_COMPOSE("spring-boot-docker-compose", RANK_C),
  ...
  SPRING_BOOT_SPRING_KAFKA("spring-boot-spring-kafka", RANK_B),
  SPRING_BOOT_SPRING_KAFKA_AKHQ("spring-boot-spring-kafka-akhq", RANK_B),
  SPRING_BOOT_SPRING_KAFKA_SAMPLE_PRODUCER_CONSUMER("spring-boot-spring-kafka-sample-producer-consumer", RANK_D),
  ...
  private static final Map<String, Seed4JCoreModuleSlug> moduleSlugMap = Stream.of(values()).collect(
    Collectors.toMap(Seed4JCoreModuleSlug::get, Function.identity())
  );
  private final String slug;
  private final Seed4JModuleRank rank;

  Seed4JCoreModuleSlug(String slug, Seed4JModuleRank rank) {
    this.slug = slug;
    this.rank = rank;
  }
  ...
  public static Optional<Seed4JModuleRank> getRank(String slug) {
    return fromString(slug).map(Seed4JCoreModuleSlug::rank);
  }

  public static Optional<Seed4JCoreModuleSlug> fromString(String slug) {
